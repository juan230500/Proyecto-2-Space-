...
def VentanaJuego(nombre):
	...
	#           _____________________________
	#__________/Diferencias entre modos de juego
	if dft:
		Aro=pygame.image.load("2.png") #Aro
		a_max=14
		l_max=600
		...
		Aro=pygame.image.load("3.png") #Enemigo
		a_max=6
		l_max=300
		l_choq=250
		desv=0
	...
	def exp(surf,x,y,xi,yi,l_choq): #Exapandir superficie (surf), se expande a x,y y se colaca siempre en xi,yi
		global in1
		if x>l_choq: #Si sobre pasa la medida 500x500 traspasa al jugaodr
			in1=0
		surf=pygame.transform.scale(surf, (x, y))
		root.blit(surf,(xi,yi))

	def ev_choque(pos1,l1,pos2,l2):
		if (pos2[0]<=pos1[0]<=pos2[0]+l2 or pos2[0]<=pos1[0]+l1<=pos2[0]+l2) and (pos2[1]<=pos1[1]<=pos2[1]+l2 or pos2[1]<=pos1[1]+l1<=pos2[1]+l2):
			print("SI")
		else:
			print("NO")
		return
	...
	while i:
		...
		if x_a>l_max: #Si el aro mide más de 600x600 se resetean sus condiciones de inicio
			x_a,y_a=50,50
			in1=1
		...
		if x_a>l_choq and in1==1:
			ev_choque((posX_jug-desv,posY_jug-desv),150+desv*2,(xi_a,yi_a),l_choq)
		

		if in1: #si debe traspasarlo, entonces se genera primero el aro
			exp(Aro,x_a,y_a,xi_a,yi_a,l_choq)
